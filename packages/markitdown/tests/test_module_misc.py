...
def test_plain_text_falls_back_from_incorrect_ascii_hint() -> None:
    # The stream-info charset can be inferred from the first chunk of text. If
    # non-ASCII UTF-8 appears later in the file, conversion should still work.
    content = ("# Title\n" + ("ascii only\n" * 500) + "cafe: café\n").encode("utf-8")
    ...
    assert "cafe: café" in result.markdown
